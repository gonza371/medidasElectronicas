...
\item[\fbox{B}] Con la punta en posición X10 y es para compensar la respuesta en frecuencia.
...
\item[\fbox{B}] En la propia punta de pruebas, y se accede mediante un destornillador de plástico.
...
\item[\fbox{B}] Funciona como un instrumento de canal único y entrada diferencial.
...
Primero debemos obtener el período de la señal y el ancho del pulso, para el período observamos que entre pulso y pulso hay 8 cuadrados, a 1ms/div, el período es de 8ms. Para el ancho del pulso, observamos que el pulso tiene una duración de 2 cuadrados, lo que significa que el ancho del pulso es de 2ms. Por lo tanto, el ciclo de trabajo se calcula como:
\begin{equation}
\text{Ciclo de trabajo} = \frac{\text{Ancho del pulso}}{\text{Período}} = \frac{2ms}{8ms} = 0.25
\end{equation}

\item[2.] El valor eficaz de la tensión.

Para calcular el valor eficaz de la tensión, primero debemos determinar la amplitud de la señal. Observamos que la señal alcanza un máximo de 4 cuadrados a 2V/div tenemos que la amplitud es de 8V. Como tenemos el ciclo de trabajo, podemos calcularlo con la siguiente fórmula:
\begin{equation}
\text{Valor eficaz} = V_{pp} \cdot \sqrt{D-D^2} 
\end{equation}
\begin{equation}
	8V \cdot \sqrt{0.25-0.25^2} = 3.46V
\end{equation}
